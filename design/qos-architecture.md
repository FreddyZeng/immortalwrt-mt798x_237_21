...
- `eqos add` 在安装限速分类前统一归一化上传/下载速度；上传为 0 时不安装上行 DSCP2，下载为 0 时不安装下行 DSCP2/MARK2。HNAT 对 mark 2 和 mark 46 使用完整 `skb->mark` 精确匹配，避免其他 fwmark 低 6 位碰撞进入 Q31/Q32/Q63。
