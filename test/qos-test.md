...
- `loadbalance` 和 `eqos add` WAN 接口绑定的 PREROUTING mark/CONNMARK 规则必须带 `-m mark ! --mark 0x8000/0x8000`，不能覆盖 SSR Plus UDP TProxy 的 `fwmark 0x8000/0x8000`（bit15 专用，避免与 eqos QoS bits[0-5] 和 mwan3 bits[0-7] 碰撞）。
